...
export function Cta({ label, href, id, track = "mentorship", className = "" }: { label: string; href: string; id: string; track?: string; className?: string }) {
  const external = /^https?:/.test(href);
  return (
    <a
      href={href}
      {...(external ? { target: "_blank", rel: "noopener noreferrer" } : {})}
      data-track={`${track}_cta_${id}`}
