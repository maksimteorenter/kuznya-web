...
function PrimaryCta({ label, id }: { label: string; id: string }) {
  return (
    <a
...
function Photo({ src, alt, ground }: { src: string; alt: string; ground: string }) {
  return (
...
export function RebootView({ L, locale }: { L: Content; locale: "ru" | "uk" }) {
  ...
  return (
    <>
      ...
      <ScrollProgress />
      <StickyRebootCTA label={L.next.stickyLabel} />
        ...
        <Container className="relative z-10 text-center">
          <div className="mx-auto max-w-3xl">
            ...
            <FadeIn delay={0.4} className="mt-9">
              <PrimaryCta label={L.next.ctaLabel} id="hero" />
      ...
      <Scene id="next" bg="bg-[#0A0706]">
        <Container className="text-center">
          ...
          <FadeIn delay={0.28} className="mt-10">
            <PrimaryCta label={L.next.ctaLabel} id="next" />
            <p className={`mt-4 text-xs uppercase tracking-[0.1em] ${text2}`}>{L.next.micro}</p>
          </FadeIn>
